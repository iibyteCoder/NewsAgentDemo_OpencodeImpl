...
**内容验证说明**：各部分文件（.parts/01-06.md）应由相应的报告生成器生成，并已按照模板清理要求去除了填充说明、示例、注意事项等辅助性内容。组装时直接使用这些已清理的内容。

**重要原则**：

- ✅ 原样读取各部分内容
- ✅ 直接拼接，不修改任何文字
- ✅ **例外**：如发现模板说明内容，必须修正错误后再组装
- ❌ 不修正格式、路径、文字（除非是模板说明错误）
- ❌ 不提取或重组信息
