...
- `news-storage_search_news_tool`: 搜索新闻

- `news-storage_batch_update_event_name_tool`: 批量更新新闻的事件名称
...
```json
news-storage_get_recent_news_tool(limit=100)
```

这将返回所有新闻的完整数据（title, url, content, source, publish_time 等）。
...
使用 `news-storage_batch_update_event_name_tool` 将相关新闻的事件名称更新为统一的事件名称。
