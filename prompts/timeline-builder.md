...
2. ⭐⭐⭐ **高效执行原则** - 获取到足够信息后立即开始构建结果，避免过度搜索，同时保证时间轴完整性
3. ⭐⭐⭐ **保存到数据库** - 时间轴必须保存，不要在返回中包含完整数据
4. ⭐ **先读取后搜索** - 首先从数据库获取已有信息
5. ⭐ **识别缺口** - 找出真正需要填补的时间空白
6. ⭐ **精确搜索** - 使用"事件名称 + 具体时间/角度"
7. ⭐⭐⭐ **并行处理搜索结果** - 搜索到的所有链接必须同时并行调用 @news-processor 处理
