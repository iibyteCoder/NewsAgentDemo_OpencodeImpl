...
- 有今日新闻 → 继续
- 有近期新闻（非今日） → 记录并继续，在输出中标注
- 3次搜索均无结果 → 返回 no_data，终止任务
...
### 4.5 事件数量不足时的补充搜索（仅当事件数 < 2时执行）

**目标**：补充至少1个新事件，确保总事件数 ≥ 2

**补充搜索策略（最多尝试2次）**：

**第1次补充搜索**：

- 关键词：类别相关子话题、具体事件名、相关人物/组织
- 时间范围：放宽到最近7天
- 目标：获取不同角度的新闻

**第2次补充搜索**（如第1次补充后仍 < 2个事件）：
...
1. 并行处理新搜索到的新闻链接（调用 @news-processor）
2. 重新聚合所有新闻（新旧新闻一起，调用 @event-aggregator）
3. 验证总事件数：
   - 总事件数 ≥ 2 → 继续
   - 总事件数 < 2 → 接受现状，继续处理（使用现有事件）
...
2. **⭐⭐⭐ 事件数量保障**
   - 目标：至少获取2个事件
   - 聚合后 < 2个事件 → 自动触发补充搜索（步骤4.5）
   - 补充搜索最多2次，使用不同策略和更宽时间范围
   - 如仍 < 2个事件 → 接受现状，继续处理现有事件
...
4. **session_id管理** - 从prompt参数获取，禁止自己生成
5. **并行处理** - 所有新闻链接和事件处理必须并行执行
6. **数据验证** - 每个步骤后检查数据，无数据立即终止
7. **模式识别** - 准确识别广泛模式vs精确模式
8. **禁止直接获取内容** - 必须通过@news-processor获取文章内容
