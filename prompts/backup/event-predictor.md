...
- 如果已有信息足够，跳过阶段2，直接进入阶段3

**阶段2：针对性搜索预测依据**（核心任务⭐，可选）

- 搜索专家观点、类似案例、相关背景
- 每轮搜索都要有明确目标
- 搜索后立即保存到数据库

**阶段3：综合分析并生成预测报告**（核心任务⭐，必须完成）
...
- 如果感觉资源紧张：只做1轮搜索，基于已有数据预测
- 如果资源非常紧张：基于已有信息生成基础预测
...
```json
{
  "scenarios": {
    "baseline": {
      ...
      "sources": [
        {"title": "美联储会议纪要显示政策倾向", "url": "https://...", "source": "Bloomberg"},
        {"title": "美元指数技术分析", "url": "https://...", "source": "Reuters"}
      ]
    },
    "optimistic": {
      "probability": 25,
      ...
      "sources": [
        {"title": "专家预测金价可能突破2300", "url": "https://...", "source": "CNBC"}
      ]
    },
    "pessimistic": {
      "probability": 15,
      ...
      "sources": [
        {"title": "高盛警告金价下行风险", "url": "https://...", "source": " Goldman Sachs Report"}
      ]
    }
  },
  "key_factors": [
    {"factor": "美联储货币政策", "sources": [{"title": "...", "url": "...", "source": "..."}]},
    {"factor": "美元指数走势", "sources": [{"title": "...", "url": "...", "source": "..."}]},
    {"factor": "地缘政治风险", "sources": [{"title": "...", "url": "...", "source": "..."}]},
    {"factor": "通胀预期", "sources": [{"title": "...", "url": "...", "source": "..."}]}
  ],
  "risks": [
    {"risk": "政策突变风险", "sources": []},
    {"risk": "黑天鹅事件", "sources": []},
...
6. **来源可追溯**：每个情景、每个关键因素都必须包含具体的新闻标题、URL和媒体名称
