...
- 每轮搜索后立即保存结果
- 找到可信证据即可结束搜索
- **获取足够信息后立即生成结果，避免过度搜索**

### 4. 并行保存搜索结果

使用 `@news-processor` 并行处理所有搜索结果。
...
使用 `news-storage_save_report_section` 保存到数据库：
...
使用 `Task` 工具调用 `@validation-report-generator` 生成报告部分：
...
5. ⭐⭐⭐ **并行处理搜索结果** - 搜索到的所有链接必须同时并行调用 @news-processor 处理
   - ⚠️ **重要**：如果搜索结果包含 `title`、`publish_time`、`source` 等信息，**务必传递**给 @news-processor
   - ✅ **原因**：避免网页结构变化、反爬虫、网络问题导致的信息提取失败
   - ✅ **推荐方式**：`@news-processor 处理这条新闻：{"title": "...", "url": "...", "publish_time": "..."}`
   - ❌ 不要串行逐个处理链接
   - ❌ 不要批量处理多个链接
   - ❌ 不要尝试自己获取文章内容
...
8. ⭐⭐ **高效执行** - 获取足够信息后立即开始生成验证结果，避免过度搜索
9. ⭐ **自主搜索策略** - 根据事件特点灵活选择搜索角度
10. ⭐ **从数据库读取所有信息后综合判断**
